...
function showAll() {{
  ...
  const secondLastCol = headers.length - 2;
  const thirdLastCol = headers.length - 3;
  headers.forEach((h, idx) => {{
    h.style.display = (idx === firstCol || idx === lastCol || idx === secondLastCol || idx === thirdLastCol) ? '' : 'none';
  }});
  rows.forEach(row => {{
    const cells = Array.from(row.querySelectorAll('td'));
    cells.forEach((cell, idx) => {{
      cell.style.display = (idx === firstCol || idx === lastCol || idx === secondLastCol || idx === thirdLastCol) ? '' : 'none';
    }});
  }});
}}
...
function filterIntradayPicks() {{
  ...
  // Show only first column (Stock) and 2nd last column (latest Delivery Qty)
  const firstCol = 0;
  const secondLastCol = headers.length - 2;
  headers.forEach((h, idx) => {{
    h.style.display = (idx === firstCol || idx === secondLastCol) ? '' : 'none';
  }});
  rows.forEach(row => {{
    const cells = Array.from(row.querySelectorAll('td'));
    cells.forEach((cell, idx) => {{
      cell.style.display = (idx === firstCol || idx === secondLastCol) ? '' : 'none';
    }});
  }});
