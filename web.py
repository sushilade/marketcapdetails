...
function showMarketCandleScript() {{
  ...
  if (!modal) {{
    ...
    modal.style.cssText = 'display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.85); z-index:10000; justify-content:center; align-items:center;';
    modal.innerHTML = `
      <div style="position:relative; width:90%; max-width:1100px; height:90%; background:#ffffff; border-radius:16px; overflow:hidden; box-shadow:0 20px 60px rgba(0,0,0,0.5); display:flex; flex-direction:column;">
        <div style="display:flex; justify-content:space-between; align-items:center; padding:15px 20px; background:linear-gradient(135deg,#1e293b 0%,#334155 100%); color:white;">
          <h3 style="margin:0; font-family:'Montserrat',sans-serif; font-size:1.1rem;"><i class="fas fa-chart-candle"></i> MARKET CANDLE - Weekly Market Cap</h3>
          <div style="display:flex; gap:10px;">
        ...
        <div style="flex:1; padding:10px;">
          <div id="marketCandleChart" style="width:100%; height:100%;"></div>
        </div>
      </div>
    `;
    document.body.appendChild(modal);
...
function buildMarketCandleChart() {{
  const table = document.getElementById('marketTable');
  if (!table) {{ alert('Market table not found'); return; }}

  const headers = Array.from(table.querySelectorAll('th'));
  const rows = Array.from(table.querySelectorAll('tbody tr'));

  // Find market cap column (latest date column containing 'market')
  let marketCapColIndex = -1;
  let marketCapColName = '';
  for (let i = headers.length - 1; i >= 1; i--) {{
    const h = headers[i].textContent.trim().toLowerCase();
    if (h.includes('market')) {{
      marketCapColIndex = i;
      marketCapColName = headers[i].textContent.trim();
      break;
    }}
  }}

  if (marketCapColIndex === -1) {{ alert('No market cap column found'); return; }}

  // Collect date -> market cap pairs
  const dataPoints = [];
  rows.forEach(row => {{
    const stockName = row.cells[0].textContent.trim();
    const dateStr = marketCapColName;
    const valStr = row.cells[marketCapColIndex].textContent.trim().replace(/,/g, '');
    const val = parseFloat(valStr);
    if (!isNaN(val)) {{
      dataPoints.push({{ date: dateStr, value: val, stock: stockName }});
    }}
  }});

  // Group by week (Monday to Friday)
  const weeklyCandles = groupDataIntoWeeklyCandles(dataPoints);
...
function groupDataIntoWeeklyCandles(dataPoints) {{
  if (dataPoints.length === 0) return [];

  // Parse the date from the column header
  const dateStr = dataPoints[0].date;
  const parsedDate = parseDateFromHeader(dateStr);
  ...
  dataPoints.forEach(dp => {{
    ...
    weeks.get(weekStart).push(dp.value);
  }});

  const candles = [];
  weeks.forEach((values, weekStart) => {{
    values.sort((a, b) => a - b);
    const mondayVal = values.length > 0 ? values[0] : 0;
    const fridayVal = values.length > 0 ? values[values.length - 1] : 0;
    const highVal = values.length > 0 ? values[values.length - 1] : 0;
    const lowVal = values.length > 0 ? values[0] : 0;
