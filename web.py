      ...
      <div style="position:relative; width:90%; max-width:1100px; height:90%; background:#ffffff; border-radius:16px; overflow:hidden; box-shadow:0 20px 60px rgba(0,0,0,0.5); display:flex; flex-direction:column;">
        <div style="display:flex; justify-content:space-between; align-items:center; padding:15px 20px; background:linear-gradient(135deg,#1e293b 0%,#334155 100%); color:white;">
          ...
          <div style="display:flex; gap:10px; align-items:center;">
            <select id="marketCandleStockSelect" onchange="buildMarketCandleChart()" style="padding:8px 12px; border:2px solid #e2e8f0; border-radius:8px; font-size:14px; background:#f8fafc; max-width:300px;"></select>
...
function buildMarketCandleChart() {{
  ...
  const select = document.getElementById('marketCandleStockSelect');
  if (select) {{
    select.innerHTML = '';
    const allOption = document.createElement('option');
    allOption.value = '';
    allOption.textContent = 'All Stocks (Aggregated)';
    select.appendChild(allOption);
    Array.from(stockDataMap.keys()).sort().forEach(stock => {{
  ...
  // Group by week
  const weeklyCandles = groupDataIntoWeeklyCandles(filteredData);
...
function groupDataIntoWeeklyCandles(dataPoints) {{
  ...
  // Parse dates from all data points
  const parsedPoints = dataPoints.map(dp => {{
    const d = parseDateFromHeader(dp.date);
    return d ? {{ date: d, value: dp.value }} : null;
  }}).filter(p => p !== null);
  ...
  parsedPoints.forEach(dp => {{
    const weekStart = getWeekStart(dp.date);
